...
INSERT INTO `users` (`id`, `firstname`, `lastname`, `email`, `phone`) VALUES ('61', 'Monserrat', 'Conner', 'linda58@example.com', '9519551625');
...
(1,1,'Est aut aliquid est. Soluta eveniet doloribus ipsam ut id assumenda quam. Consequuntur et velit aperiam error corporis rem corporis. Est similique quisquam quia maxime eaque possimus facere officiis. Odio magni atque dicta voluptatibus hic iste delectus.','a.avi',5,NULL,'1991-11-16 08:14:38.000','2019-06-19 18:11:58.000')
...
,(3,7,'Nihil ut et reprehenderit et. Numquam veniam quis impedit. Voluptas saepe rerum illo vel omnis. Aliquam illum ut quo saepe ea unde est.','non.mp4',704558,NULL,'1988-09-17 20:56:38.000','2019-06-19 18:11:58.000')
,(4,8,'Sint quod fugit molestiae dolore repellendus est. Quis corporis necessitatibus commodi placeat temporibus doloremque voluptatem. Nemo maxime sit dolorem perferendis repellat. Et impedit deleniti eaque pariatur ut quia voluptatibus.','earum.mp4',65108,NULL,'1995-10-07 10:19:53.000','2019-06-19 18:11:58.000')
...
('15','1','15','1974-02-07 20:53:55'),
('16','1','16','1973-05-11 03:21:40'),
('17','1','17','2008-12-17 13:03:56'),
('18','1','18','1995-07-17 21:22:38'),
('19','1','19','1985-09-07 23:34:21'),
('20','1','20','1973-01-27 23:11:53')
; 
...
-- 1. Подсчитать общее количество лайков, которые получили пользователи младше 12 лет.
	-- (выводит сумму лайков, если пользователю на сегодня ещё не исполнилось 12 лет)
        
SELECT COUNT(id) AS 'Кол-во лайков'
FROM likes 
WHERE user_id IN 
	(
	SELECT user_id 		
	FROM `profiles` 
	WHERE birthday 
		BETWEEN DATE(CURRENT_DATE() - INTERVAL 12 YEAR) 
...
SELECT p.gender AS 'Гендер', COUNT(p.gender) AS 'Кол-во лайков'
FROM likes l
JOIN `profiles` p
...
SELECT id AS 'ID', CONCAT(firstname, ' ', lastname) AS 'Пользователи, не отправлявшие сообщения'
FROM users
...
-- Найдем пользователя с максимальным количеством полученных сообщений:
SELECT to_user_id AS 'ID получателя', COUNT(to_user_id) AS 'Кол-во полученных сообщений' 
FROM messages 
GROUP BY (to_user_id)
LIMIT 1;

-- Возьмем пользователя с id = 1, так как у него больше всего сообщений от разных людей:
SELECT 
